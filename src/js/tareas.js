    ...
    async function agregarTarea(tarea) {
        ...
        try {
            ...
            if (resultado.tipo === 'exito') {
                ...
                }, 740);  // Cerrar el modal después de 1 segundo
    ...
    async function eliminarTarea(tarea) {
        ...
        try {
            ...
            if (resultado.resultado) {
                ...
                const tareaElemento = document.querySelector(`[data-tarea-id="${id}"]`);
                if (tareaElemento) {
                    tareaElemento.remove();
                }

                // Actualizar la lista de tareas en memoria
                tareas = tareas.filter(tareaMemoria => tareaMemoria.id !== id);

                // Mostrar alerta de éxito
                Swal.fire('Eliminado!', resultado.mensaje, 'success');

                // Opcional: Si prefieres recargar la página después de la eliminación
